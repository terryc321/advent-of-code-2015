...
rangeOn r (Bulb p b) = if inRange r p then Bulb p On
                                      else Bulb p b

rangeToggle :: Range -> Bulb -> Bulb
rangeToggle r (Bulb p b) = if inRange r p
                      then case b of
                             Off -> Bulb p On
                             On -> Bulb p Off
                      else Bulb p b
...
f1 :: Bulb
f1 = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
     in rangeOn r b0

-- f2 :: Bulb
-- f2 = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
--      in rangeOff r b0

f3 :: Bulb
f3 = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
     in rangeToggle r b0

f4 :: Bulb
f4 = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
         b1 = rangeToggle r b0
         b2 = rangeToggle r b1
     in b2
        
f5 :: Bulb
f5 = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
         b1 = rangeToggle r b0
         b2 = rangeToggle r b1
         b3 = rangeToggle r b2
     in b3
...
f6 :: Bulb -> [Bulb]
f6 b = let r = mkRange (Pt 3 3) (Pt (-1) (-1))
           b1 = rangeToggle r b
           b2 = rangeToggle r b1
           b3 = rangeToggle r b2
       in [b,b1,b2,b3]
...
ex1 :: Bulb
ex1 = tunnel input (Bulb (Pt 0 0) Off)
...
rec = let x = 0
          y = 0
          count = 0
          fn = \x1 -> \y1 -> \s1 -> let out = (tunnel input (Bulb (Pt x1 y1) Off))
                                    in case out of
                                         (Bulb _ On) -> s1 + 1
                                         (Bulb _ Off) -> s1
...
we can set execution timing on ghc interpreter
ghci> :set +s

ghci> main
Hello, Haskell!
solution = [ 543903 ] 
(128.01 secs, 200,535,608,976 bytes)

almost 60 times slower in interpreter than compiled 
-}
