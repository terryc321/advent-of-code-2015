...
(define (explain-fn e)
  (cond
   ((and (pair? e) (eq? (car e) 'cast-drain)) (format #t "player casts drain~%~a~%" (car (cdr e))))
   ((and (pair? e) (eq? (car e) 'cast-magic-missile)) (format #t "player casts magic missile~%~a~%" (car (cdr e))))
   ((and (pair? e) (eq? (car e) 'cast-shield)) (format #t "player casts shield~%~a~%" (car (cdr e))))
   ((and (pair? e) (eq? (car e) 'cast-poison)) (format #t "player casts poison~%~a~%" (car (cdr e))))
   ((and (pair? e) (eq? (car e) 'cast-recharge)) (format #t "player casts recharge~%~a~%" (car (cdr e))))
...
(define (play1c s)
  (cond ;; cannot cast any spells - die 
   ((< (state-mana s) 53)
	...
	;;(format #t "~a~%" s2)
      'died))
   ((and *max-mana-spent* (> (state-mana-spent s) *max-mana-spent*))
    'too-costly) ;; if max mana spent defined - too costly then
   (#t
...
(define (example1)
  ...
  (let ((mana 250)
	...
	(shield 0)
	(poison 0)
	(boss-hits 13) ;; boss has 13 hit points with boss damage 8 
	(mana-spent 0)
	(moves '()))
    (let ((s (make-state mana armour hits recharge shield poison boss-hits mana-spent moves)))
      (let ((s2 (state-moves! s (list `(start ,(state-without-moves s))))))
...
(define (example2)
  ...
  (let ((mana 250)
	...
	(shield 0)
	(poison 0)
	(boss-hits 14) ;; boss has 14 hit points now - again with boss damage 8 
	(mana-spent 0)
	(moves '()))
    (let ((s (make-state mana armour hits recharge shield poison boss-hits mana-spent moves)))
      (let ((s2 (state-moves! s (list `(start ,(state-without-moves s))))))
...
example2 does not run ,,, does not find a solution ?? why

M = magic missile spell
R = recharge spell 
S = shield spell
D = drain spell
P = poison spell 
