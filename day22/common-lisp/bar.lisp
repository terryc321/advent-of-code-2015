;; only at start of the players turn - not the boss turn - you get on hit point deducted
;; BAR> (run)
;; new best - win player with 1501 mana
;; new best - win player with 1461 mana
;; new best - win player with 1408 mana
;; new best - win player with 1295 mana
;; new best - win player with 1289 mana
;; accepted
...
(defun boss-move (s)
  (let ((s (copy-state s)))
    ;;(decf (state-hits s))   ;; player turn to mean just the PLAYER not the BOSS??
    (cond
      ((<= (state-hits s) 0) 'player-died)
      ((> (state-mana-spent s) *best*) 'overspend)    
      (t
