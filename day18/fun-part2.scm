...
#|
replace all references to hash-ref with corner-hash-ref that will
make hash look like all corners are stuck on
|#
(defmacro corner-hash-ref (h pos)
  `(cond
    ((equal? ,pos `(1 1)) #t)
    ((equal? ,pos `(100 1)) #t)
    ((equal? ,pos `(1 100)) #t)
	     ...
	     (foo (lambda (x y)
		    (let ((neighbour 0))
		      ...
		      "A light which is off turns on if exactly 3 neighbors are on, and stays off otherwise."
                      (let ((on (corner-hash-ref hash (list x y))))
			(cond
	     ...
	     (ck (lambda (x y)
		   (cond
		    ...
		    (#t
		     ...
		     (ck (+ x 1) y))))))
      (ck 1 1)
      (hash-set! res (list 1 1) #t)
      (hash-set! res (list 100 1) #t)
      (hash-set! res (list 1 100) #t)
      (hash-set! res (list 100 100) #t)      
      res)))
...
four corners stuck on ...

after step 98 : #<hash-table 7f9d1d0103c0 866/1759> 
after step 99 : #<hash-table 7f9d1d010280 907/1759> 
after step 100 : #<hash-table 7f9d1d010160 886/1759>  <<------ 886
after step 101 : #<hash-table 7f9d1d010040 908/1759> 
$50 = #f
